            ...
            switch (question.QuestionType) {
                ...
                case 'points':
                    questionScore = selectedOptionIds.reduce((score, optId) => {
                        ...
                        return score + (option ? option.Score : 0);
                    }, 0);
...
exports.getQuizResult = async (req, res) => {
    try {
        ...
        if (result.UserID !== userId && 
            result.quiz.lesson.course.InstructorID !== userId) {
            return res.status(403).json({ 
        ...
        if (questions && Array.isArray(questions)) {
            for (const [index, q] of questions.entries()) {
                ...
                if (q.options && Array.isArray(q.options)) {
                    await Promise.all(q.options.map((opt, optIndex) => 
                        db.Option.create({
                            QuestionID: question.QuestionID,
                            OptionText: opt.text,
        ...
        for (const quiz of quizzes) {
            ...
            const attemptStats = await db.Result.findAll({
                ...
                attributes: [
                    ...
                    [
                        sequelize.literal(
                            `SUM(CASE WHEN Score >= ${quiz.PassingScore} THEN 1 ELSE 0 END)`
                        ),
                        'passCount'
                    ]
                ],
    ...
    try {
                    ...
                    include: [
                        {
                            model: db.Option,
                            as: 'options',
                            attributes: userId ? ['OptionID', 'Text', 'IsImage'] : 
        ...
        if (userId) {
            userAttempts = await db.Result.findAll({
                ...
                attributes: [
                    'ResultID', 'Score', 'AttemptDate', 'TimeTaken',
                    [
        ...
        const response = {
            ...
            statistics: {
                ...
                passRate: statistics[0].get('attemptCount') > 0 
                    ? (parseInt(statistics[0].get('passCount')) / parseInt(statistics[0].get('attemptCount'))) * 100 
                    : 0
            }
        };
        ...
        if (userAttempts) {
            ...
            response.canAttempt = quiz.MaxAttempts === 0 || 
...
exports.update = async (req, res) => {
    ...
    try {
        ...
        if (hasAttempts > 0 && Status === 'published') {
            ...
            const attemptedUpdate = restrictedFields.some(field => 
