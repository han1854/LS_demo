...
exports.create = async (req, res) => {
  ...
  try {
    ...
    // Transform user data
    const plainEnrollment = enrollmentWithData.get();
    if (plainEnrollment.user) {
      plainEnrollment.user = buildUserResponse(plainEnrollment.user);
    }

    res.status(201).json(plainEnrollment);
  } catch (error) {
    await transaction.rollback();
...
exports.findAll = async (req, res) => {
  try {
    ...
    // Transform user data in enrollments
    const transformedEnrollments = enrollments.map(enrollment => {
      const plainEnrollment = enrollment.get();
      if (plainEnrollment.user) {
        plainEnrollment.user = buildUserResponse(plainEnrollment.user);
      }
      if (plainEnrollment.course && plainEnrollment.course.instructor) {
        plainEnrollment.course.instructor = buildUserResponse(plainEnrollment.course.instructor);
      }
