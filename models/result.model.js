module.exports = (sequelize, DataTypes) => {
      ...
      beforeCreate: async (result) => {
        ...
        if (quiz && quiz.Duration) {
          result.ExpiresAt = new Date(
            result.StartedAt.getTime() + (quiz.Duration * 60 * 1000)
          );
        }
      },
  ...
  // Instance methods
  Result.prototype.calculateTimeRemaining = function() {
  ...
  Result.prototype.addAnswer = async function(questionId, answer, transaction) {
  ...
  Result.prototype.complete = async function(transaction) {
    if (this.Status !== 'in_progress') {
