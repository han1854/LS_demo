module.exports = (sequelize, DataTypes) => {
  const Question = sequelize.define("Question", {
    ...
    QuestionType: {
      ...
      comment: 'single=single choice, multiple=multiple choice, points=points per option, text=free text, matching=match pairs'
    },
    QuestionText: { 
      type: DataTypes.TEXT,
      allowNull: false,
    ...
    Metadata: {
      type: DataTypes.JSON,
      allowNull: true,
      comment: 'Additional metadata specific to question type (e.g., matching pairs, text validation)'
    },
    CreatedAt: {
      type: DataTypes.DATE,
      ...
      beforeValidate: async (question) => {
        ...
        switch (question.QuestionType) {
          ...
          case 'multiple':
            ...
            if (optionCount < 2) {
              throw new Error(`${question.QuestionType} choice questions must have at least 2 options`);
            }
          ...
          case 'matching':
            ...
            if (!question.Metadata || !Array.isArray(question.Metadata.pairs) || 
  ...
  Question.prototype.validateMultipleChoice = async function(selectedOptions) {
    ...
    const allCorrectSelected = correctOptions.every(o => 
      selectedOptions.includes(o.OptionID)
    );
  ...
  Question.prototype.validateTextAnswer = async function(answer) {
    ...
    const userAnswer = validation.caseInsensitive ? 
      answer.toLowerCase() : answer;
    const correctAnswer = validation.caseInsensitive ? 
      validation.answer.toLowerCase() : validation.answer;
