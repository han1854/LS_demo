...
const { validateUser, validateProfileUpdate, validateLogin } = require('../middleware/userValidation');
const { authMiddleware, checkRole } = require('../middleware/auth');

// Public Authentication Routes
router.post('/register', validateUser, userController.register);
router.post('/login', validateLogin || ((req,res,next)=>next()), userController.login);
...
router.get('/my/progress', authMiddleware, userController.getMyProgress);
router.get('/my/certificates', authMiddleware, userController.getMyCertificates);
router.get('/my/assignments', authMiddleware, userController.getMyAssignments);

// User Preferences & Settings
router.get('/preferences', authMiddleware, userController.getPreferences);
router.put('/preferences', authMiddleware, userController.updatePreferences);
router.put('/notifications/settings', authMiddleware, userController.updateNotificationSettings);
...
router.post('/become-instructor', authMiddleware, userController.applyForInstructor);

// Admin User Management
router.get('/', authMiddleware, checkRole(['admin']), userController.findAll);
router.get('/:id', authMiddleware, checkRole(['admin']), userController.findOne);
router.put('/:id', authMiddleware, checkRole(['admin']), validateProfileUpdate, userController.update);
router.delete('/:id', authMiddleware, checkRole(['admin']), userController.delete);

// User Status Management (Admin)
router.put('/:id/status', authMiddleware, checkRole(['admin']), userController.updateStatus);
router.put('/:id/role', authMiddleware, checkRole(['admin']), userController.updateRole);
router.put('/:id/verify', authMiddleware, checkRole(['admin']), userController.verifyUser);
